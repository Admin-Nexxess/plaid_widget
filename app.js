...
console.log(":58 PLAID_ENV", PLAID_ENV);
console.log(":59 PLAID_ENV", APP_PORT);

plaid_environment = PlaidEnvironments.sandbox;
if (PLAID_ENV == "production") {
  plaid_environment = PlaidEnvironments.production;
}

console.log(":66 PLAID_ENV", PLAID_ENV);
console.log(":67 PLAID_WEBHOOK_URL", PLAID_WEBHOOK_URL);
...
console.log(":79 configuration", configuration);

const client = new PlaidApi(configuration);

console.log(":83 configuration", configuration);
const app = express();
...
// This is an endpoint defined for the OAuth flow to redirect to.
app.get('/oauth-redirect.html', function (request, response, next) {
  response.sendFile('./views/oauth-redirect.html', { root: __dirname });
});
...
app.post('/api/create_link_token', async function (request, response) {
  console.log ("app.js:116 create link", request.body);
  let plaid_products = request.body.plaid_products.split(
    ',',
  );;
  // plaid_products = "transactions,auth,liabilities";
  // console.log("app.js:121 create link", request);
  const configs = {
    user: {
  ...
  console.log ("app.js:137 configs", configs);
  ...
  //   configs.PLAID_WEBHOOK_URL = PLAID_ANDROID_PACKAGE_NAME;
  // }

  if (request.body.plaid_webhook_url !== '') {
    console.log (":148 request.body.plaid_webhook_url", request.body.plaid_webhook_url)
    configs.webhook = request.body.plaid_webhook_url;
  }

  console.log ("app.js:152 configs", configs);

  try {
    const createTokenResponse = await client.linkTokenCreate(configs);
    prettyPrintResponse(":151");
    prettyPrintResponse(createTokenResponse);
    response.json(createTokenResponse.data);
  } catch (error) {
    // prettyPrintResponse({"error":"155"});
...
app.post('/api/set_access_token', async function (request, response, next) {
  ...
  receivedRedirectUri = request.body.receivedRedirectUri;
  try {
...
app.get('/api/auth', async function (request, response, next) {
  try {
    ...
    console.log("app.js:245 api/auth", authResponse);
    response.json(authResponse.data);
  } catch (error) {
    prettyPrintResponse(error.response);
    console.log("app.js:249 api/auth", error);
    return response.json(formatError(error.response));
  }
});
...
// app.get('/api/institutions', async function (request, response, next) {
...
//     response.json({ error: null, institutions });
//   } catch (error) {
//     // Handle error
//   }
...
app.get('/api/institutions', async function (request, response, next) {
  ...
  try {
    const transactionsResponse = await client.institutionsGet(configs);
    prettyPrintResponse(transactionsResponse);
    response.json(transactionsResponse.data);
  } catch (error) {
    prettyPrintResponse(error.response);
